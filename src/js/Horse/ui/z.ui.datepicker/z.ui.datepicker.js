...
define([],function () {
    ...
    datepicker.prototype = {
        ...
        validateParams: function () {
            ...
            if (Horse.validate.isExistAttr(id, "datepicker")) {
                alert("使用日期插件缺少datepicker属性");
                return false;
            }
            if (this.options.mindate != "") {
                if (!Horse.validate.isDate(this.options.mindate)) {
                    alert("使用日期插件的mindate参数格式不正确！");
                    return false;
                }
            }
            if (this.options.maxdate != "") {
                if (!Horse.validate.isDate(this.options.maxdate)) {
        ...
        _registerDatePickerElementClick: function () {
            var handleEvent = Horse.util.delegate(this._handleDatePickerElementClick, this);
        ...
        _registerTbodyTdClick: function () {
            ...
            for (var i = 0; i < tdList.length; i++) {
                var td = tdList[i];
                var handleEvent = Horse.util.delegate(datepicker.prototype._handleTbodyTdClick, this, [{ options: options, td: td }]);
        ...
        _registerBtnClear: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnClear, this);
            document.getElementById("btnClear").onclick = handleEvent;
        },
        //注册关闭事件
        _registerBtnClose: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnClose, this);
            document.getElementById("btnClose").onclick = handleEvent;
        },

        //注册选择今天按钮的事件
        _registerBtnToday: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnToday, this);
        ...
        _registerDocumentClick: function () {
            ...
            document.onclick = function (event) {
                var handleEvent = Horse.util.delegate(datepicker.prototype._handleDocumentClick, this, [{ event: event, options: options }]);
                handleEvent();
            };
        ...
        _registerBtnLastYear: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnLastYear, this);
            document.getElementById("lastyear").onclick = handleEvent;
        },

        //注册下一年事件
        _registerBtnNextYear: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnNextYear, this);
            document.getElementById("nextyear").onclick = handleEvent;
        },

        //注册上一月事件
        _registerBtnLastMonth: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnLastMonth, this);
            document.getElementById("lastmonth").onclick = handleEvent;
        },

        //注册下一月事件
        _registerBtnNextMonth: function () {
            var handleEvent = Horse.util.delegate(this._handleBtnNextMonth, this);
        ...
        _handleDatePickerElementClick: function () {
            ...
            if (dpvalue === "") {
                ...
                if (!Horse.validate.isDate(dpvalue)) {
        ...
        _handleTbodyTdClick: function (params) {
            var tdList = document.getElementById("dpcontrol").getElementsByTagName("tbody")[0].getElementsByTagName("td");
            for (var i = 0; i < tdList.length; i++) {
                Horse.util.removeClass(tdList[i], "datepicker-dateselected");
            }
            Horse.util.addClass(params.td, "datepicker-dateselected");
        ...
        _handleDocumentClick: function (params) {
            ...
            if(Horse.validate.objIsNull(dateselected)){
            	return;
            }
            var dateselectedValue = dateselected.value;
            if (dateselectedValue.trim() != "") {
                if (!Horse.validate.isDate(dateselectedValue)) {
                    alert("输入的日期格式不正确！正确格式为'yyyy-MM-dd'！");
