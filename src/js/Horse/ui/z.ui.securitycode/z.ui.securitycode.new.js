 ...
 *date：2016-07-04
 */
define(['text!virtualKeyTempl', "virtualKey"], function(VirtualKeyTempl, virtualKey) {
	...
	securityCodePlugIn.prototype = {
		...
		renderTempl: function() {
			...
			var temp = "";
			temp += "<div class='secutitycodeArea' id='" + securityCodePlugInAreaId + "'>";
			temp += "	<div class='secutitycodeBg'>";
			temp += "		<div class='item' sc='1'></div>";
			temp += "		<div class='item' sc='2'></div>";
			temp += "		<div class='item' sc='3'></div>";
			temp += "		<div class='item' sc='4'></div>";
			temp += "		<div class='item' sc='5'></div>";
			temp += "		<div class='item' sc='6'></div>";
			temp += "		<input type='password' style='display: none' />";
			temp += "	</div>";
			temp += "	<div class='virtualKeyContainer' id=" + virtualKeyContainerId + "></div>";
			temp += "</div>";

			$('#' + containerId).html(temp);
		...
		click_document: function() {
			...
			$(document).bind("click", function(e) {
				var target = $(e.target);
				if (target.closest("#" + t.options.securityCodePlugInAreaId + ",#" + t.options.referencePasswordId + ",#" + t.options.fixedPositionAreaId).length == 0) {
					$("#"+t.options.fixedPositionAreaId).hide();
					return;
				}
			});
		},
		...
		click_secutitycode: function() {
			Horse.validate.isFunction(this.options.clickSCCallback) && this.options.clickSCCallback();
			if (this.options.isHasFixedPosition) {
		...
		virtualkeyClickCallbackHandler: function(currentOp) {
			...
			if (currentOp != "delete") {
				...
				Horse.validate.isFunction(this.options.virtualkeyClickCallback) && this.options.virtualkeyClickCallback();
				return true;
			}
			//获取倒序
		...
		fillInCompleteCallBackHandler: function(pwd) {
			//判断输入完成后是否需要回调函数
			Horse.validate.isFunction(this.options.fillInCompleteCallBack) && pwd.length == 6 && this.options.fillInCompleteCallBack(pwd);
		},

		/// TODO 确认密码
		needConfirmPasswordHandler: function(pwd) {
			//判断是否需要确认密码
			if (!Horse.validate.isNull(this.options.needConfirmPassword.referencePasswordId)) {
				...
				if (refPwd.length == 6 && pwd.length == 6 && refPwd == pwd) {
					Horse.validate.isFunction(this.options.needConfirmPassword.callback) && this.options.needConfirmPassword.callback(true, refPwd);
					return;
				}
				Horse.validate.isFunction(this.options.needConfirmPassword.callback) && this.options.needConfirmPassword.callback(false);
			}
		},
